...
function generateProductsJs(products, currConfig, imageExts) {
    const items = products.map((p, i) => {
        const ext = imageExts ? imageExts[i] : '.svg';
        return `    { id: "product-${i}", name: ${JSON.stringify(p.name)}, description: ${JSON.stringify(p.description || '')}, price: ${p.price}, maxUnits: ${p.maxUnits || 10}, image: "product-${i}${ext}" }`;
    });
...
async function build(opts, distDir) {
    ...
    if (isV2) {
        ...
        // Copy product images and track extensions
        const imageExts = products.map((p, i) => {
        ...
        fs.writeFileSync(path.join(shopTmp, 'products.js'), generateProductsJs(products, currConfig, imageExts));
